...
from libs . iooooooOoO0 import logger , dump_json , localize , KODI_VERSION , set_setting , ISESTUARYPALANTIR , load_json , get_setting , CACHE , WINDOW
...
class ContextMenu ( object ) :
 def __init__ ( self , url = None ) :
  if url :
   self . IsEstuaryPalantir = True
  else :
   self . IsEstuaryPalantir = False
   try :
    url = xbmc . getInfoLabel ( 'Container.ListItem().FolderPath' ) . split ( '?' ) [ 1 ]
   except : pass
  ...
  if 97 - 97: Iii1i
  if ii1i != 'albums' :
   ii1i = o0OoOOo00OOO . getMediaType ( )
 ...
 def _generate_menu ( self ) :
  ...
  if self . itemContext [ 'content' ] == 'movies' :
   ...
   if not self . itemContext [ 'isWidget' ] and not ISESTUARYPALANTIR :
   ...
   Oo0ooo0OO0 = False
   if WINDOW . getProperty ( 'p3PelissFavoritasPath' ) :
    try :
     I1IIiiIiI1iI = P3Item ( ) . fromurl ( WINDOW . getProperty ( 'p3PelissFavoritasPath' ) . split ( '?' ) [ 1 ] )
   ...
   if self . itemContext [ 'tmdb' ] in O0OO00OO0O :
  ...
  elif not self . itemContext [ 'isWidget' ] and not ISESTUARYPALANTIR :
 ...
 def _exec ( self , labelAction ) :
  ...
  O0 = None
  Ii1Ii1 = 5000
  I111i1i11iII = 'special://home/addons/plugin.video.palantir3/icon.png'
  ...
  if OO0OOOoOOooO == 'set_view' :
   ...
   xbmcgui . Dialog ( ) . notification ( 'Palantir 3' , iIi )
   ...
   xbmc . executebuiltin ( "RunPlugin(plugin://plugin.video.palantir3/?%s)" % i11I1IiIiI1i . tourl ( ) )
   ...
   if self . itemUrl and self . itemUrl . trailer :
    ...
    try :
     ...
     if OoOo0 :
      ...
      if searchYou ( self . itemContext [ 'title' ] + ' Trailer Espa̱ol' ) :
       return
    except :
     pass
     if 23 - 23: Iii1i
    xbmcgui . Dialog ( ) . ok ( 'Palantir 3 %s' % ( localize ( 30065 ) ) , localize ( 30066 ) % self . itemContext [ 'title' ] )
    if 17 - 17: IiIIii11Ii
 ...
 fanart = self . itemContext . get ( 'fanart' ) )
   ...
   IIIi11ii1Ii = 'plugin://plugin.video.palantir3/?%s' % i11I1IiIiI1i . tourl ( )
   ...
   try :
    ...
    if not self . itemContext [ 'isWidget' ] :
     ...
     if oo00 :
      ...
      iIIIiII1 = "{0},|,{1}|{2}" . format ( self . itemContext [ 'tmdb' ] , self . itemContext [ 'tmdb' ] , self . itemContext [ 'tmdb' ] )
      o00o . sql = re . sub ( iIIIiII1 , '' , o00o . sql , re . MULTILINE )
      if 42 - 42: i1i1i1111I % OooOoo . iI / i1i1i1111I . ooOOO
      if o00o . sql . endswith ( '()' ) :
       xbmc . executebuiltin ( 'Action(back)' )
      else :
       oo00 = 'plugin://plugin.video.palantir3/?%s' % o00o . tourl ( )
 ...
 type = xbmcgui . INPUT_PASSWORD )
   ...
   if i1iiiIii :
    ...
    xbmcgui . Dialog ( ) . ok ( 'Palantir 3' , localize ( 30524 ) )
   ...
   if xbmcgui . Dialog ( ) . yesno ( 'Palantir 3' , localize ( 30535 ) ) :
    with Watched ( ) as OoOOo0OoOOO0 :
 ...
 self . itemContext [ 'tmdb' ] , self . itemContext [ 'season' ] , self . itemContext [ 'episode' ] )
  ...
  if O0 :
   logger ( O0 )
   ii1II1 = "Palantir %s" % xbmcaddon . Addon ( ) . getAddonInfo ( 'version' )
