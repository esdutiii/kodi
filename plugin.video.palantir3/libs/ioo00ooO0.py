...
from libs . iooooooOoO0 import logger , compare_versions , KODI_VERSION , ISESTUARYPALANTIR , get_setting , CACHE , localize
...
class iI111iiIi11i ( xbmcgui . WindowXMLDialog ) :
 ...
 def __new__ ( cls , p3Item , duration ) :
  return super ( iI111iiIi11i , cls ) . __new__ ( cls , "Custom_1111_Overlay.xml" , 'special://home/addons/skin.estuary.palantir' )
 ...
 def onInit ( self ) :
  ...
  try :
   ...
   self . getControl ( 200000 ) . setImage ( 'special://home/addons/plugin.video.palantir3/icon.png' )
...
class P3Player ( xbmc . Player ) :
 ...
 def playStream ( self , p3Item , videoUrl , listitem , enlace = None ) :
  ...
  if ISESTUARYPALANTIR and self . p3Item . content == 'episodes' :
 ...
 self . videoUrl . url )
   ...
   if o0oOoo0O0o0Oo and ( xbmc . getSkinDir ( ) == 'skin.estuary.palantir' ) and ( compare_versions ( xbmcaddon . Addon ( 'skin.estuary.palantir' ) . getAddonInfo ( 'version' ) , '1.3.0' ) >= 0 ) :
    if 15 - 15: ooo000
    if 63 - 63: o00o0OO00O
    with open ( xbmcvfs . translatePath ( 'special://profile/addon_data/skin.estuary.palantir/settings.xml' ) , 'r' ,
 ...
 method = 'DELETE' , headers = { 'Authorization' : 'Bearer %s' % get_setting ( 'Realdebrid_token' ) } )
   ...
   try :
    downloadpage (
 url = 'https://api.alldebrid.com/v4/user/history/delete?agent=Palantir3&apikey=%s' % get_setting (
 'Alldebrid_apikey' ) )
   except : pass
   if 9 - 9: oOo0O00 * i1
