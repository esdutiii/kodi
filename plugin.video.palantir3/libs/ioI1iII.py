...
def setInfoTag ( listitem , item ) :
 ...
 if KODI_VERSION >= 20.0 :
  ...
  if item . tmdb :
   iI111iiIi11i . setUniqueIDs ( { 'tmdb' : item . tmdb } , "tmdb" )
  if item . duration :
 ...
 'mediatype' : Ooo }
  ...
  if item . tmdb :
   listitem . setUniqueIDs ( { 'tmdb' : item . tmdb } , "tmdb" )
  if item . rating :
...
class P3Item ( object ) :
 ...
 def getart ( self ) :
  ...
  if 'fanart' not in self . __dict__ :
   self . __dict__ [ 'fanart' ] = 'special://home/addons/plugin.video.palantir3/fanart.gif'
  ...
  for I1i1i , iII11iIi1iIiI in enumerate ( [ 'poster' , 'icon' , 'fanart' , 'thumb' , 'clearlogo' , 'season_poster' , 'still' ] ) :
   if iII11iIi1iIiI in self . __dict__ :
    ...
    if iIIiii1iI :
     if not ( iIIiii1iI . lower ( ) . startswith ( 'http' ) or iIIiii1iI . startswith ( 'special://home/addons/plugin.video.palantir3' ) ) :
 ...
 def fromurl ( self , url ) :
  self . __dict__ . update ( eval ( p3b64decode ( url ) ) )
  return self
